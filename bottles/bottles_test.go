...
Take one down and pass it around, 98 bottles of beer on the wall.`
	...
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Generator.Verse() mismatch (-want +got):\n%s", diff)
	}
}
...
Take one down and pass it around, 97 bottles of beer on the wall.`
	...
	got := gen.Verses(99, 98)

	println(got)

	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Generator.Verses() mismatch (-want +got):\n%s", diff)
	}
}
...
func TestTheWholeSong(t *testing.T) {
	gen := NewGenerator()
...
Go to the store and buy some more, 99 bottles of beer on the wall.`
	...
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Generator.Song() mismatch (-want +got):\n%s", diff)
	}
}
